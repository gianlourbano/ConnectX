...
public class Evaluator {
	...
	public int eval(CXBoard board) {
		int[] firstPlayerScore    = {0, 0, 0, 0, 0};
		int[] secondPlayerScore = {0, 0, 0, 0, 0};
		
		firstPlayerScore[0]    = evalWins(board, 0);
		secondPlayerScore[0] = evalWins(board, 1);
		
		firstPlayerScore[1]    = evalThreats(board, 0);
		secondPlayerScore[1] = evalThreats(board, 1);
		
		firstPlayerScore[2]    = evalOpenEnds(board, 0);
		secondPlayerScore[2] = evalOpenEnds(board, 1);
		
		firstPlayerScore[3]    = evalPositionWeights(board, 0);
		secondPlayerScore[3] = evalPositionWeights(board, 1);
		
		firstPlayerScore[4]    = evalSevenTraps(board, 0);
		secondPlayerScore[4] = evalSevenTraps(board, 1);
		
		int score = 0;
		
		for (int i = 0; i < firstPlayerScore.length; i++) {
			score += (evalWeights[i] * (firstPlayerScore[i] - secondPlayerScore[i]));
		}
